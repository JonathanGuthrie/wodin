 ...
 */
Sasl *SaslChooser(ImapSession *session, const insensitiveString &residue)
{
    Sasl *result;
	...
	{
	    result = new SaslCramMD5(session);
	}
	else
	{
	    if (0 == residue.compare("DIGEST-MD5"))
	    {
		result = new SaslDigestMD5(session);
	    }
	    else
	    {
#endif /* 0 */
		if (session->GetServer()->IsAnonymousEnabled() && (0 == residue.compare("ANONYMOUS")))
		{
		    result = new SaslAnonymous(session);
		}
		else
		{
		    result = NULL;
...
Sasl::status SaslPlain::ReceiveResponse(const std::string &csLine2)
{
	...
	if ('\0' == line[0])
	{
	    ...
	    ImapUser *userData = session->GetServer()->GetUserInfo(author.c_str());
	    if (userData->CheckCredentials(password.c_str()))  // Note: Assuming "not secure"
	    {
		authorizationEntity = author;
		currentStatus = ok;
	    }
	    else
	    {
