...
MailStoreMbox::MailStoreMbox(ImapSession *session) : MailStore(session) {
    ...
    m_isDirty = false;
    m_inboxPath = session->user()->expandPath("/var/mail/%n");
    m_homeDirectory = new std::string(session->user()->homeDir());
}
...
MailStore::MAIL_STORE_RESULT MailStoreMbox::createMailbox(const std::string &FullName) {
	...
	if (result == SUCCESS) {
		...
		if (ENOENT == errno) {
			...
			if (0 != (m_errnoFromLibrary = writeMailboxMetadataFile(fullPath.c_str(), m_session->master()->fqdn().c_str(), m_session->user()->uid()))) {
			    result = MAILBOX_PATH_BAD;
			}
		    }
		}
...
MailStore::MAIL_STORE_RESULT MailStoreMbox::renameMailbox(const std::string &SourceName, const std::string &DestinationName) {
	...
	if (0 == rename(sourcePath.c_str(), destPath.c_str())) {
	    if (isInbox) {
		writeMailboxMetadataFile(sourcePath.c_str(), m_session->master()->fqdn().c_str(), m_session->user()->uid());
	    }
	}
	else {
	    result = GENERAL_FAILURE;
...
MailStore::MAIL_STORE_RESULT MailStoreMbox::addMessageToMailbox(const std::string &FullName, const uint8_t *data, size_t length,
	...
	try {
		...
		createTime.format(DateTime::FROM_LINE);
		*m_outFile << "From " << m_session->user()->name() << "@" << m_session->master()->fqdn() << " " << createTime.str() << "\n";
		*m_outFile << "X-Status: ";
...
unsigned MailStoreMbox::serialNumber() {
    return m_uidLast + 1;
}
...
void MailStoreMbox::mailboxList(const std::string &pattern, MAILBOX_LIST *result, bool listAllMailboxes) {
    uid_t savedUserId = geteuid();
    seteuid(m_session->user()->uid());
    if (listAllMailboxes) {
...
MailMessage::MAIL_MESSAGE_RESULT MailStoreMbox::messageData(MailMessage **message, unsigned long uid) {
	...
	*message = m_messageIndex[msn-1].messageData;
	(*message)->messageFlags(m_messageIndex[msn-1].flags);
    }
    else {
...
size_t MailStoreMbox::bufferLength(unsigned long uid) {
    unsigned long msn = mailboxUidToMsn(uid);
    size_t result = 0;
...
const SEARCH_RESULT *MailStoreMbox::searchMetaData(uint32_t xorMask, uint32_t andMask, size_t smallestSize, size_t largestSize, DateTime *beginInternalDate, DateTime *endInternalDate) {
	...
	i->messageData->messageFlags(i->flags);
...
MailStore::MAIL_STORE_RESULT MailStoreMbox::lock(void) {
    return MailStore::SUCCESS;
}


MailStore::MAIL_STORE_RESULT MailStoreMbox::unlock(void) {
    return MailStore::SUCCESS;
}
