...
ImapHandler *deleteHandler(ImapSession *session, INPUT_DATA_STRUCT &input) {
  (void) input;
  return new DeleteHandler(session, session->parseBuffer());
}
 ...
 */
IMAP_RESULTS DeleteHandler::receiveData(INPUT_DATA_STRUCT &input) {
